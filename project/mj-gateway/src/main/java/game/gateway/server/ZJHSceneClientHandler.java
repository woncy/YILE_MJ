 ...
 *
 */
public class ZJHSceneClientHandler implements PxMsgHandler<Object>{
	 private final static Logger log = LoggerFactory.getLogger(GameServerHandler.class);
	    ...
	    private ZJHSceneClient ZJHSceneClient;
	    
	    public ZJHSceneClientHandler(Runnable connectCallback) {
    ...
    public void onConnect(Session<Object> session) throws Exception {
        log.info("连接场景服务器成功！");
        gatewayService.onSceneConnect(ZJHSceneClient);
        connectCallback.run();
    }
	...
	public void onDisconnect(Session<Object> arg0) throws Exception {
		 	log.info("场景连接断开!！");
	        gatewayService.onSceneDisconnect(ZJHSceneClient);
	}
	...
	public void onMessage(PxMsg pxMsg) throws Exception {
		 gatewayService.handlerZJHScene(pxMsg);//未完成
	}
	
	public void setZJHSceneClient(ZJHSceneClient ZJHSceneClient) {
        this.ZJHSceneClient = ZJHSceneClient;
    }
