...
public class CheckDelDouniuRoomMsg extends AbstractSessionPxMsg {
    ...
    private List<DouniuSceneUserInfo> infos = new ArrayList<>();
    private boolean isEnd;
    
    public CheckDelDouniuRoomMsg() {
        super(ID);
    }
	...
	public void decode(ByteBuf in, ChannelHandlerContext ctx) throws Exception {
        ...
        int len = in.readByte();
        for (int i = 0; i < len; i++) {
            DouniuSceneUserInfo item = new DouniuSceneUserInfo();
            item.decode(in, ctx);
            infos.add(item);
        }
	...
	public void encode(ByteBuf out) throws Exception {
		 out.writeShort(sceneId);
	        ...
	        for (int i = 0; i < infos.size(); i++) {
	        	DouniuSceneUserInfo item = infos.get(i);
	            item.encode(out);
	        }
	...
	public List<DouniuSceneUserInfo> getInfos() {
		return infos;
	}


	public void setInfos(List<DouniuSceneUserInfo> infos) {
		this.infos = infos;
	}
	...
	public String toString() {
		return "CheckDelDouniuRoomMsg [sceneId=" + sceneId + ", roomId=" + roomId + ", userId=" + userId + ", result="
