...
//        }
        ...
        winindexs = indexs;
        ArrayList<Pai>[] faPaiByControl = FaPaiUtil.faPaiByControl(new int[]{}, userPlaces.length, paiPool.getPais(), huier);
        for (int i = 0; i < userPlaces.length; i++) {
        	ArrayList<Pai> arrayList = faPaiByControl[i];
        	for (int j = 0; j < arrayList.size(); j++) {
				userPlaces[i].addShouPai(arrayList.get(j));
			}
        }
        ...
        if(!isGang){
	        if(!isContins(currentIndex, winindexs)){
	        	...
	        	if(tingPais2!=null){
					...
					log.info("kazhang:{}+gl="+nextDouble);
					if (nextDouble * 100 < 70) {
						int[] pais = userPlace.checkKaZhang();
						log.info("kazhang:{}+"+Arrays.toString(pais));
						if(pais!=null){
							pai = paiPool.changeHuPai(pai, pais);
						}
					}
	        	}
...
//                    		}else if(AgariUtils.isHuPai(current.getShouPai(), pai)){
                			checkResult.setHu(true);
                		}else{
