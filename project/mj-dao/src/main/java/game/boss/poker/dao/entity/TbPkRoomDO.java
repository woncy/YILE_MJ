...
public class TbPkRoomDO extends EntityObject<TbPkRoomDO, TbPkRoomDO.Key>{
	...
	public TbPkRoomDO(int createUserId,int userMax,String uuid,String roomCheckId,java.util.Date startDate,java.util.Date endDate,boolean start,boolean end,int version,int sceneId,int chapterNums,mj.data.Config config,int initScore) {
	...
	@Override
	public String toString() {
		return "TbPkRoom[id:"+ id+",createUserId:"+ createUserId+",userMax:"+ userMax+",uuid:"+ (uuid == null ?"null":uuid.substring(0, Math.min(uuid.length(), 64)))+",roomCheckId:"+ (roomCheckId == null ?"null":roomCheckId.substring(0, Math.min(roomCheckId.length(), 64)))+",startDate:"+ startDate+",endDate:"+ endDate+",start:"+ start+",end:"+ end+",version:"+ version+",sceneId:"+ sceneId+",chapterNums:"+ chapterNums+",config:"+ config+",initScore:"+ initScore+ "]";
	}
	...
	public static final class Table extends TableInfo<TbPkRoomDO ,Key>{
		...
		private Table(){
			...
			super.initProperty("initScore", "initScore", int.class, new TypeReference<Integer>() {});
		}
		...
		@Override public String getInsertSql(){
			return "INSERT INTO `tb_pk_room` (`create_user_id`,`user_max`,`uuid`,`room_check_id`,`start_date`,`end_date`,`start`,`end`,`version`,`scene_id`,`chapter_nums`,`config`,`initScore`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
		}

		@Override public String getReplaceSql(){
        	return "REPLACE INTO `tb_pk_room` (`id`,`create_user_id`,`user_max`,`uuid`,`room_check_id`,`start_date`,`end_date`,`start`,`end`,`version`,`scene_id`,`chapter_nums`,`config`,`initScore`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
        }
		...
		@Override public String getUpdateSql(){
			return "UPDATE `tb_pk_room` SET `create_user_id`=?,`user_max`=?,`uuid`=?,`room_check_id`=?,`start_date`=?,`end_date`=?,`start`=?,`end`=?,`version`=?,`scene_id`=?,`chapter_nums`=?,`config`=?,`initScore`=? WHERE `id`=?";
		}
